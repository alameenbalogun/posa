-- POSA cloud identity bridge.
-- Keeps legacy/offline local business IDs while linking them to Supabase UUIDs.

create table if not exists public.cloud_business_links (
  local_business_id text primary key,
  cloud_business_id uuid not null unique references public.businesses(id) on delete cascade,
  owner_user_id uuid not null references auth.users(id) on delete cascade,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

alter table public.cloud_business_links enable row level security;
drop policy if exists cloud_business_links_owner on public.cloud_business_links;
create policy cloud_business_links_owner on public.cloud_business_links
  for select using (owner_user_id = auth.uid() or public.is_member(cloud_business_id));
...
begin
  ...
  where local_business_id = p_local_business_id
  ...
  if v_cloud_business is null then
    ...
    values (
      v_cloud_business,
      coalesce(p_business->>'name', 'POSA business'),
      p_business->>'legal_name',
      p_business->>'phone',
      p_business->>'email',
      p_business->>'address',
  ...
  values (
    v_branch_id, v_cloud_business, coalesce(p_branch->>'name', 'Main Shop'),
  ...
  values (
    p_device->>'id', v_cloud_business, v_branch_id, coalesce(p_device->>'name', 'Front Counter'),
    coalesce(p_device->>'platform', 'web'), coalesce(p_device->>'fingerprint', p_device->>'id'),
    coalesce(p_device->>'app_version', '1.0.0')
  )
  on conflict (id) do update set
    business_id = excluded.business_id, branch_id = excluded.branch_id,
